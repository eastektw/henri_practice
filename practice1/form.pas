...
begin
  ...
  begin
    ...
    AStringGrid.ColWidths[ColIndex]:=MaxColWidth+AStringGrid.Canvas.TextWidth('A');{I want the width of column not only fit the content in it but also expand the width of one character, so I choose the character "A"}
  end;
end;
...
procedure TForm1.ReadArrayIntoStringGrid(AFileData: TCOMMANDS_array;
...
begin
  ...
  begin
    ...
    NowColIndex:=3;{after loading the content to column 0, 1, and 2 the next empty column in the same row is 3}
...
procedure TForm1.ReadTListIntoStringGrid(AFileData: TList;
  AStringGrid: TStringGrid);
var
  Rows, Cols, NowColIndex, ColsForCommandLine:Integer;
  IndexOfAFileData, IndexOfParam:Integer;
...
procedure TForm1.ReadLinkedListIntoStringGrid(AFileData: PCOMMAND_Ptr;
  AStringGrid: TStringGrid);
var
